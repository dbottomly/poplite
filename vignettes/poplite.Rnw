...
of 'samples' in 'dna' as opposed to the combination of 'sample_id' and 'wave' (though 'sample_id' will still be kept as it links to the 'clinical' table).  Where the primary key is a column of integer values that uniquely identifies each row and by default it was added
...
table being limited to only those values in common between the two tables.  For example specifying the sample_id+wave relationship between 'samples' and 'dna' will result in the 'dna' table being limited to only those with DNA specimens that have sample IDs and wave values
...
{
    vcf.rows <- rowData(vcf.obj)
    ref.dta <- cbind(
                    seqnames=as.character(seqnames(vcf.rows)),
                    as.data.frame(ranges(vcf.rows))[,c("start", "end")],
                    ref=as.character(mcols(vcf.rows)$REF),
...
populate.allele.table <- function(vcf.obj)
{
    ...
    vcf.rows <- rowData(exp.obj)
    ref.dta <- cbind(
                    seqnames=as.character(seqnames(vcf.rows)),
                    as.data.frame(ranges(vcf.rows))[,c("start", "end")],
                    ref=as.character(mcols(vcf.rows)$REF),
                    alt=as.character(mcols(vcf.rows)$ALT),
                    stringsAsFactors=FALSE
                    )
    return(ref.dta)
}
...
vcf.sc <- append(vcf.sc, allele.sc)
...
populate.samp.alt.table <- function(vcf.obj)
{
    ...
    ret.dta <- cbind(
                        ...
                        allele_count=sapply(strsplit(temp.vrange$GT, "\\|"), function(x) sum(as.integer(x), na.rm=T)),
                        stringsAsFactors=F
                        )
...
vcf.sc <- append(vcf.sc, geno.all.sc)

@

Our relationships can be specified as before.  One wrinkle here is that there is a dependence in the table structures here that is more complicated than the sample tracking tables in the 'Basic Usage' section above.  The first
...
side indicates the primary key of the 'from' table.
...
Now that our schema is complete, we can populate our database with these three tables.  In this case, we can populate the entire database with one statement, however for demonstration purposes
we will divide the \texttt{VCF} object into several pieces and populate them one at a time.
...
try(pop.res <- poplite::select(vcf.db, .tables=tables(vcf.db)))

vrange.tab <- as(vcf[1:1000], "VRanges")
