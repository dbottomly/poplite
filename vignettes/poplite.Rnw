...
the file path to where the database resides.  We then call the \texttt{populate} method and provide it with our named \texttt{data.frames}.  The database is now ready to be queried!
...
Addition of methods for setting constraints as well as indicating whether INSERT should ignore duplicate rows.  This is already supported internally so can be done in a pinch.

Support for between table 'OR' statements in \texttt{filter}.
